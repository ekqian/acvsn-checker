...
error_messages = {
    ...
    "REPORTING_ERROR": lambda reporting:
    f"Reporting attribute ({click.style(reporting, fg='red')}) is not valid.",

    "WL_ERROR": lambda wave_length:
    f"Wave length attribute ({click.style(wave_length, fg='red')}) is not valid.",

    "MEASUREMENT_RH_ERROR": lambda relative_humidity:
    f"MeasurementRH attribute ({click.style(relative_humidity, fg='red')}) is not valid.",

    "SIZING_TECHNIQUE_ERROR": lambda sizing_technique:
    f"SizingTechnique attribute ({click.style(sizing_technique, fg='red')}) is not valid.",

    "SIZE_RANGE_ERROR": lambda size_range:
    f"SizeRange attribute ({click.style(size_range, fg='red')}) is not valid.",

    "MEASURE_DIR_ERROR": lambda measurement_direction, valid_list:
    f"MeasurementDirection attribute ({click.style(measurement_direction, fg='red')}) is not valid; must be "
    f"{click.style(', '.join([str(s) for s in valid_list]), fg='green')}",

    "SPECTRAL_COV_ERROR": lambda spectral_coverage, valid_list:
    f"SpectralCoverage attribute ({click.style(spectral_coverage, fg='red')}) is not valid; must be "
    f"{click.style(', '.join([str(s) for s in valid_list]), fg='green')}",
    ...
    f"WLMode attribute ({click.style(wl_mode, fg='red')}) is not valid; must be "
    f"{click.style(', '.join([str(s) for s in valid_list]), fg='green')}",
...
class StandardName:
    ...
    def check_aermp(self) -> bool:
        ...
        if not valid_size_range:
            self.error_codes.append(error_messages["SIZE_RANGE_ERROR"](size_range))
        if not valid_reporting:
            self.error_codes.append(error_messages["REPORTING_ERROR"](reporting))
    ...
    def check_aeropt(self) -> bool:
        ...
        if not valid_core_name:
            self.error_codes.append(error_messages["CORE_NAME_ERROR"](core_name, self.measurement_cat))
        if not valid_wl:
            self.error_codes.append(error_messages["WL_ERROR"](wave_length))
        if not valid_rh:
            self.error_codes.append(error_messages["MEASUREMENT_RH_ERROR"](relative_humidity))
        if not valid_size_range:
            self.error_codes.append(error_messages["SIZE_RANGE_ERROR"](size_range))
